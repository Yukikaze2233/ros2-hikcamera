 ...
 *  @~chinese
 *  @brief  初始化SDK资源
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 在调用其他接口之前调用该接口进行初始化SDK资源。
 ...
 *  @brief  结束SDK调用，销毁资源
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 该接口调用后，SDK会释放申请的所有资源；如果要继续使用SDK接口，需要先调用 MV_FG_Initialize() 或者重新枚举采集卡。
 ...
 *  @remarks 进行采集卡操作之前必须调用该接口，只有在调用该接口时内部采集卡列表才会更新。 \n
             采集卡类型说明如下：\n
                | 序号 | 采集卡类型定义 | 值 | 说明 |
                | :---:| :---:| :---:| :---:|
                | 1 | \ref MV_FG_GEV_INTERFACE | 0x00000001 | GigE Vision采集卡 |
                | 2 | \ref MV_FG_CAMERALINK_INTERFACE | 0x00000004 | Camera Link采集卡 |
                | 3 | \ref MV_FG_CXP_INTERFACE | 0x00000008 | CoaXPress采集卡 |
                | 4 | \ref MV_FG_XoF_INTERFACE | 0x00000010 | XoFLink采集卡 |
 *  @~english
...
/********************************************************************//**
 *  @~chinese
 *  @brief  打开采集卡，导入配置文件（配置文件来自 MV_FG_FeatureSave() 接口）
 *  @param  nIndex                  [IN]           采集卡索引，从0开始
 ...
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 该接口定义了如下几种取流策略，可以根据实际需求进行选择。具体描述如下：\n
        \li OneByOne：从旧到新一帧一帧的从输出缓存列表中获取图像，创建流通道后默认为该策略\n
        \li LatestImagesOnly：仅从输出缓存列表中获取最新的一帧图像，同时清空输出缓存列表\n
        \li LatestImages：从输出缓存列表中获取最新的图像，不清空输出缓存列表\n
        \li UpcomingImage：在调用取图接口时清空输出缓存列表中所有图像，并等待相机即将生成的一帧图像。
        \li bUseTrashBuffer：当bUseTrashBuffer=true时, 如果注册的缓存全部被取走且没有调用 MV_FG_ReleaseFrameBuffer() \ MV_FG_QueueBuffer() \ MV_FG_FlushQueue() 将缓存放回，
 ...
 *  @remarks 在回调函数中处理耗时操作会阻塞后续帧信息获取。\n
             注册回调方式和 MV_FG_GetFrameBuffer() 互斥。\n
             必须在调用 MV_FG_StartAcquisition() 前注册回调函数。
 ...
 *  @remarks 在回调函数中处理耗时操作会阻塞后续帧信息获取。\n
             注册回调方式和 MV_FG_GetFrameBuffer() 互斥。\n
             必须在调用 MV_FG_StartAcquisition() 前注册回调函数。 \n
             @note 当bBufferRelease为0时，buffer不会自动释放，需要及时调用 MV_FG_ReleaseFrameBuffer 释放buffer。如果内部buffer用完，无法继续取流。
 ...
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 需要调用 MV_FG_ReleaseFrameBuffer() 接口进行缓存信息的释放。
 ...
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 该接口用于释放不再使用的图像缓存，与 MV_FG_GetFrameBuffer() 配套使用。使用 MV_FG_GetFrameBuffer() 接口取到的图像数据pstBufferInfo，需要用 MV_FG_ReleaseFrameBuffer() 接口进行权限释放。
 ...
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 在获取到缓存信息之后，调用 MV_FG_ReleaseFrameBuffer() 或者 MV_FG_QueueBuffer() 之前调用该接口才能得到有效信息。
 ...
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 图像缓存大小通过 MV_FG_GetPayloadSize() 获取；私有信息由用户自定义。
 ...
 *  @param  phBuffer                [OUT]          缓存句柄
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 图像缓存大小通过 MV_FG_GetPayloadSizeEx() 获取；私有信息由用户自定义。
...
/********************************************************************//**
 *  @~chinese
 *  @brief  从流通道中销毁由 MV_FG_AnnounceBuffer() 注册的缓存
 *  @param  hStream                 [IN]           流通道句柄
 *  @param  hBuffer                 [IN]           缓存句柄
 *  @param  pBuffer                 [OUT]          图像缓存地址
 *  @param  pPrivate                [OUT]          私有信息地址
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
 *  @remarks 只能撤销未使用队列中的缓存，可通过 MV_FG_FlushQueue() 将缓存放到未使用队列。
...
/********************************************************************//**
 *  @~chinese
 *  @brief  获取一帧图像的缓存信息（使用 MV_FG_AnnounceBuffer() 注册缓存的取流方式）
 *  @param  hStream                 [IN]           流通道句柄
 *  @param  phBuffer                [OUT]          缓存句柄
 *  @param  nTimeout                [IN]           超时时间，以毫秒为单位
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
...
*  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。
*  @remarks 在回调函数中处理耗时操作会阻塞后续帧信息获取。\n
            注册回调方式和 MV_FG_GetImageBuffer() 互斥。\n
            必须在调用 MV_FG_StartAcquisition() 前注册回调函数。
 ...
 *  @return 成功，返回\ref 状态码 "MV_FG_SUCCESS"；失败，返回\ref 状态码 "状态码" 。 
 *  @remarks 该接口仅支持 MV_FG_PIXEL_TYPE_RGB8_Packed 和 PixelType_Gvsp_BGR8_Packed 像素格式
 ...
 *  @remarks 支持图像旋转，图像翻转，图像分割，但需搭配特定前置条件如：\n
             图像旋转和图像翻转：
			 \li 支持的像素格式：MV_FG_PIXEL_TYPE_Mono8、MV_FG_PIXEL_TYPE_RGB8_Packed和MV_FG_PIXEL_TYPE_BGR8_Packed。

             图像分割:
			 \li 支持任意像素格式。
			 \li 需搭配线阵相机的“MultiLightControl”节点使用。您可通过节点设置多个不同的曝光值，如MultiLightControl=2。\n
			 相机会将2个不同曝光值所对应的两张图像交叠合并为一张图像（实际高度为2张图像的高度）发送给上层应用程序。\n
             之后调用该接口并传入重组方式为RECONSTRUCT_MODE_SPLIT_BY_LINE_2，可将相机发送的一张图像分割为2张图像，这2张图像分别对应一个曝光值。\n
                @note 
                若使用普通相机或未打开线阵相机的“MultiLightControl”节点，则图像分割无意义，只是将图像按行分割为2，3，4张图像，\n
                此时每张图像的高度变为原图像的1/2，1/3，1/4(由enReconstructMode决定)。
